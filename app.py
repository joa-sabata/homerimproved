...
# === INICIALIZACIÓN ABSOLUTA DE LA MEMORIA (SESSION STATE) ===
...
# ================= SECCIÓN ENTRADA =================
# Usamos una clave dinámica que cambia al borrar, forzando a vaciar la caja por completo
...
# LÓGICA ESTRICTA DEL BOTÓN BORRAR TOTAL (LIMPIEZA DE MEMORIA)
if btn_borrar:
    st.session_state.contador_reset += 1  # Cambia la clave de la caja de texto para vaciarla
    st.session_state.texto_salida = ""    # Borra los resultados procesados anteriores de raíz
    st.rerun()                             # Recarga instantáneamente la app limpia
            ...
            if url_match:
                ...
                if numeros_en_producto:
                    for num in reversed(numeros_en_producto):
                        if (len(num) == 5 and num.startswith('3')) or (len(num) == 5 and num.startswith('28')) or num.startswith('249'):
                            continue
                        codigo_manual = num
                        break

                # --- 2. EXTRACCIÓN DE CANTIDAD AISLANDO EL CÓDIGO MANUAL ---
                texto_analisis_cantidad = " ".join(lineas[i:fin_contexto])
                texto_analisis_cantidad = re.sub(r'\$\s*\d+[\d.,]*', '', texto_analisis_cantidad)
                ...
                for num in todos_los_numeros:
                    if (len(num) == 5 and num.startswith('3')) or (len(num) == 5 and num.startswith('28')) or num.startswith('249'):
                        continue
                    if codigo_manual and num == codigo_manual:
                        continue
                    cantidad = num
                    break
